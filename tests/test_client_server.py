...
from tecscipyModbusTCP.client import DeviceIdentificationResponse, ModbusClient
...
class TestClientServer(unittest.TestCase):
    ...
    def test_read_write_requests(self):
        ...
        for addr in [0x0000, 0x1234, 0x2345, 0x10000 - MAX_WRITABLE_BITS]:
            ...
            self.assertRaises(
                ValueError, self.client.write_multiple_coils, addr, bits_l
            )
        ...
        self.assertRaises(
            ValueError, self.client.write_multiple_coils, 0xFFF0, [False] * 17
        )
        ...
        for addr in [0x0000, 0x1234, 0x2345, 0x10000 - MAX_READABLE_BITS]:
            ...
            self.assertEqual(
                self.client.read_discrete_inputs(addr, len(bits_l)), bits_l
            )
            ...
            self.server.data_bank.set_discrete_inputs(addr, bits_l)
            self.assertRaises(
                ValueError, self.client.read_discrete_inputs, addr, len(bits_l)
            )
        ...
        for addr in [0x0000, 0x1234, 0x2345, 0x10000 - MAX_WRITABLE_REGS]:
            ...
            self.assertEqual(
                self.client.read_holding_registers(addr, len(words_l)), words_l
            )
            # holding registers space: multi-write at max size
            words_l = [randint(0, 0xFFFF) for _ in range(MAX_WRITE_READ_REGS)]
            self.assertEqual(
                self.client.write_read_multiple_registers(
                    addr, words_l, addr, len(words_l)
                ),
                words_l,
            )
            self.assertEqual(
        ...
        self.assertRaises(
            ValueError, self.client.write_multiple_registers, 0x1000, [0x10000]
        )
        self.assertRaises(
            ValueError, self.client.write_multiple_registers, 0xFFF0, [0] * 17
        )
        self.assertRaises(
            ValueError,
            self.client.write_read_multiple_registers,
        ...
        for addr in [0x0000, 0x1234, 0x2345, 0x10000 - MAX_READABLE_REGS]:
            ...
            self.assertEqual(
                self.client.read_input_registers(addr, len(words_l)), words_l
            )
            # input registers space: multiple read/write over sized
            words_l.append(randint(0, 0xFFFF))
            self.server.data_bank.set_input_registers(addr, words_l)
            self.assertRaises(
                ValueError, self.client.read_input_registers, addr, len(words_l)
            )
    ...
    def test_server_strength(self):
        ...
        for func_code in range(0x80):
            if func_code not in SUPPORTED_FUNCTION_CODES:
                ...
                self.assertEqual(
                    self.client.custom_request(bytes([func_code, 0x00])), None
                )
    ...
    def test_client_read_identification(self):
        ...
        self.server.device_id = DeviceIdentification(
            ...
            major_minor_revision=maj_min_rev,
        ...
        if not dev_id_resp:
            self.fail(
                "ModbusClient.read_device_identification() method failed unexpectedly"
            )
        else:
            # return DeviceIdentificationResponse on success
            self.assertEqual(
                isinstance(dev_id_resp, DeviceIdentificationResponse), True
            )
            # check read data
